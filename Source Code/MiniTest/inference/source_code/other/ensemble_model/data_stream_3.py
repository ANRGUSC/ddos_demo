...
def data_stream(ns, node, k, attack_ratio, attack_duration, results_df, df):
    ...
    data_all_nodes = data_all_nodes.sort_values(by=["TIME"]).reset_index(drop=True)

    benign_que = collections.deque(maxlen=1000000)
    attack_que = collections.deque(maxlen=1000000)
    for index, row in data.iterrows():
        cur_time = data.loc[index, "TIME"]
        tmp_data = data_all_nodes.loc[data_all_nodes["TIME"] == cur_time]
        for tmp_index, tmp_row in tmp_data.iterrows():
            if tmp_row["PACKET"] > 0 and tmp_row["init_pred"] == 0:
                benign_que.append(tmp_row["PACKET"])
            elif tmp_row["PACKET"] > 0 and tmp_row["init_pred"] == 1:
                attack_que.append(tmp_row["PACKET"])
...
def main_data_stream():
    ...
    output_path = "./Output/Data_Stream/Test_Results/data_stream_with_pred_final_3_days_1000000_que_with_correlation.csv"
...
def plot_error_vs_k_estimate(data, output_path):
    data = data.sort_values(by=["ATTACK_PARAMETER"]).reset_index(drop=True)
    k_list = data["ATTACK_PARAMETER"].unique()
    mse_list = []
    for k in k_list:
        tmp = data.loc[data["ATTACK_PARAMETER"] == k]
        tmp["a"] = 1
        if k < 0.3:
            tmp["k_estimate"] = (tmp["k_estimate"] < 0.3).astype(int)
        else:
            tmp["k_estimate"] = (tmp["k_estimate"] >= 0.3).astype(int)
        error = accuracy_score(tmp["a"], tmp["k_estimate"])

        # mse = mean_squared_error(tmp["ATTACK_PARAMETER"], tmp['k_estimate'])

        # mse = r2_score(tmp["ATTACK_PARAMETER"], tmp['k_estimate'])
        mse_list.append(error)
    plt.clf()
    plt.plot(k_list, mse_list, marker="o")
    plt.xlabel("Attack Parameter (k)")
    plt.ylabel("Model Accuracy Selection")
...
def main_generate_metrics():
    dataset_path = "./Output/Data_Stream/Test_Results/data_stream_with_pred_final_3_days_1000000_que_with_correlation.csv"
...
def main_plot_error_vs_k_estimate():
    dataset_path = "./Output/Data_Stream/Test_Results/data_stream_with_pred_final_3_days_1000000_que_with_correlation.csv"
    data = pd.read_csv(dataset_path)
    output_path = "./Output/Data_Stream/Metrics/error_k_estimate.png"
    plot_error_vs_k_estimate(data, output_path)
