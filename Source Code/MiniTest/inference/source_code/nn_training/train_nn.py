...
def create_nn_model(
    ...
    if nn_model == "dense":
        ...
        model.add(
            tf.keras.layers.Dense(
                output_shape, activation="sigmoid", bias_initializer=output_bias
            )
        )
    elif nn_model == "cnn":
        if (
        ...
        model.add(
            tf.keras.layers.Dense(
                256, input_shape=(input_shape,), activation="relu", trainable=False
            )
        )
...
def main_train_model(
    ...
    metadata_path,
    metadata_metric,
    num_selected_nodes_for_correlation,
    run_number,
    group_number,
):
    seed = 1
    ...
    for node in nodes:
        ...
        if (
            ...
            elif metadata_metric == "CORRELATION":
                metadata_dataset_all = load_dataset(metadata_path)
                metadata_dataset = metadata_dataset_all.loc[
                    (metadata_dataset_all["NODE_1"] == node)
                ]
    ...
    if len(sys.argv) > 1:
        ...
        if metadata_metric == "SHAP":
            metadata_path = (
                ...
                + "run_"
                + str(run_number)
                + "/report/shap/feature_importance/feature_importance.csv"
            )
        else:
            metadata_path = metadata_path.replace(
                "Output/metadata", "Output/group_" + str(group_number) + "/metadata"
            )
        main_train_model(
            nn_model,
            architecture,
        ...
        metadata_path_list = [
            ...
            + "pre_process/Output/group_"
            + str(group_number)
            + "/metadata/feature_importance.csv",
        ]
