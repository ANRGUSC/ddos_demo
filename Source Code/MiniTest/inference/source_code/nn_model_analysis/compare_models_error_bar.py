        ...
        for nn_model in nn_model_list:
            for architecture in architecture_list:
                ...
                for group_number in range(CONFIG.NUM_GROUPS):
                    ...
                    num_runs = len(list(os.listdir(dir_report_path))) - 1
...
def plot_metric_vs_attack_parameter(
    ...
    for metric in metrics:
        ...
        for i, mean_report_path in enumerate(all_mean_report_path):
            ...
            mean_report = mean_report.groupby("k")
            means = mean_report.mean()
            errors = mean_report.sem()
            plt.errorbar(
                means.index,
                means[metric],
                yerr=1.96 * errors[metric],
                color=colors[i],
...
def main_plot_compare_nn_models_architectures(metric, mode):
    ...
    for nn_model in nn_model_list:
        ...
        for architecture in architecture_list:
            ...
            # metric_mean_report_path = input_path + 'mean_report_combined_' + metric + '_' + mode + '.csv'
            metric_mean_report_path = (
                input_path + "general_report_combined_" + metric + "_" + mode + ".csv"
            )
