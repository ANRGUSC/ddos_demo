...
def main_fit(benign_data_path):
    ...
    attacked_data_path = (
        ...
        + "N_BaIoT/Data/SimpleHome_XCS7_1003_WHT_Security_Camera_mirai_attack_udp_aggregation_Source-IP_time_window_10-sec_stat_Number.csv"
    )
...
def main_fit_cauchy(benign_data_path):
    ...
    a = tfp.substrates.numpy.distributions.TruncatedCauchy(
        loc=benign_cauchy[0],
        scale=benign_cauchy[1],
        low=0,
        high=benign_data["PACKET"].max(),
    )
    print(a)
...
def main_plot_pdf(benign_data_path, output_path):
    ...
    plt.clf()
    num_bins = 50
    count_empirical, bins_count_empirical = np.histogram(
        benign_data["PACKET"], bins=num_bins
    )
    pdf_empirical = count_empirical / sum(count_empirical)
    plt.bar(
        bins_count_empirical[1:],
        count_empirical / benign_data["PACKET"].shape[0],
        label="Emprical",
    )
