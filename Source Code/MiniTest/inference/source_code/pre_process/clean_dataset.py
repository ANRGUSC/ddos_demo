...
def create_anonymization_map(init_data, dataset_info, output_path):
    ...
    for node in nodes:
        ...
        if node not in dataset_info.index:
            temp = pd.DataFrame(
                {
                    "NODE_2": str(dataset_info.shape[0]),
                    "LAT": mean_lat,
                    "LNG": mean_lng,
                    "LAT_2": mean_lat_2,
                    "LNG_2": mean_lng_2,
...
def combine_main_dataset_with_info_and_anonymize(
    init_data, dataset_info, init_data_output_path, anonym_data_output_path
):
    ...
    init_data["TIME"] = pd.to_datetime(init_data["TIME"])
    delta_time = init_data["TIME"][0] - datetime(
        2021,
        1,
        1,
        init_data["TIME"][0].hour,
        init_data["TIME"][0].minute,
        init_data["TIME"][0].second,
    )
    init_data["TIME_2"] = init_data["TIME"] - delta_time

    init_data = init_data[
        ["NODE", "NODE_2", "LAT", "LNG", "LAT_2", "LNG_2", "TIME", "TIME_2", "ACTIVE"]
    ]
    init_data.to_csv(init_data_output_path, index=False)

    anonym_data = init_data[["NODE_2", "LAT_2", "LNG_2", "TIME_2", "ACTIVE"]]
    anonym_data = anonym_data.rename(
        columns={"NODE_2": "NODE", "LAT_2": "LAT", "LNG_2": "LNG", "TIME_2": "TIME"}
    )
...
def create_benign_dataset(
    ...
    # nodes = [15385, 23089, 31973, 13101, 11903, 15890, 18235, 14129, 7245, 27898, 3399, 32800, 29502,
    #         2848, 3139, 4860, 9713, 18061, 6882, 8300]
    benign_data = pd.DataFrame(
...
def main_generate_original_data():
    """The main function for calling the create_original_dataset function."""

    dataset_info_output_path = (
        CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/dataset_info/dataset_info.csv"
    )
    prepare_output_directory(dataset_info_output_path)
    dataset_info = load_dataset_info(
        CONFIG.PARKING_METER_INFO_PATH, dataset_info_output_path
    )

    init_data_output_path = (
        CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/init_data/init_data.csv"
    )
    prepare_output_directory(init_data_output_path)
    init_data = load_main_dataset(
        CONFIG.PARKING_METER_DATASET_PATH, init_data_output_path
    )

    anonym_data_map_output_path = (
        CONFIG.OUTPUT_DIRECTORY
        + "pre_process/Output/dataset_info/dataset_info_anonym.csv"
    )
    dataset_info = create_anonymization_map(
        init_data, dataset_info, anonym_data_map_output_path
    )

    anonym_data_output_path = (
        CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/init_data/anonym_data.csv"
    )
    # init_data = init_data.head(100)
    init_data, anonym_data = combine_main_dataset_with_info_and_anonymize(
        init_data, dataset_info, init_data_output_path, anonym_data_output_path
    )

    # use the anonymized dataset
    init_data = anonym_data

    # set the desired begin and end date of the output dataset
    begin_date = init_data["TIME"][0]
    end_date = init_data["TIME"][init_data.shape[0] - 1]

    sliced_data = slice_dataset(init_data, begin_date, end_date)
    sliced_data_output_path = (
        CONFIG.OUTPUT_DIRECTORY
        + "pre_process/Output/original_data/original_data_"
        + str(begin_date)
        + "_"
        + str(end_date)
        + ".csv"
    ...
    prepare_output_directory(sliced_data_output_path)
    sliced_data.to_csv(sliced_data_output_path, index=False)
