...
def generate_features_time_slot(
    training_data_rows, data, time, nodes, durations, training_data
):
    ...
    row["ATTACK_PARAMETER"] = data.loc[0, "ATTACK_PARAMETER"]
    row["TIME"] = time
    ...
    for node in nodes:
        for duration, duration_name in durations.items():
            ...
            row["ACTIVE_" + str(duration_name)] = data.loc[select_rows, "ACTIVE"].mean()
            row["PACKET_" + str(duration_name)] = data.loc[select_rows, "PACKET"].mean()

        row["NODE"] = node
        row["ATTACKED"] = data.loc[
            (data["NODE"] == node) & (data["TIME"] == time), "ATTACKED"
        ].values[0]
...
def generate_features(
    attacked_dataset_path, output_path, data_type, num_train_days, num_test_days
):
    ...
    times = list(
        set(
            attacked_data.loc[
                (attacked_data["TIME"] >= begin_date)
                & (attacked_data["TIME"] < end_date),
                "TIME",
            ]
        )
    )
    ...
    durations = {
        ...
        timedelta(hours=1): "1hr",
        timedelta(hours=2): "2hr",
        timedelta(hours=4): "4hr",
        timedelta(hours=8): "8hr",
        timedelta(hours=16): "16hr",
        timedelta(hours=24): "24hr",
        timedelta(hours=30): "30hr",
        timedelta(hours=36): "36hr",
        timedelta(hours=42): "42hr",
        timedelta(hours=48): "48hr",
    }
    ...
    for duration, duration_name in durations.items():
        template_column.append("ACTIVE_" + str(duration_name))
    for duration, duration_name in durations.items():
        template_column.append("PACKET_" + str(duration_name))
    ...
    training_data = training_data.sort_values(by=["NODE", "TIME"])
...
def main_generate_features(data_type, num_train_days, num_test_days):
    ...
    path = (
        ...
        + "pre_process/Output/attacked_data/"
        + data_type
        + "/*.csv"
    )

    output_directory = (
        CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/" + data_type + "_data/"
    )
...
def main_combine_data(data_type):
    ...
    if data_type == "train":
        ...
        output_path = (
            CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/train_data/train_data.csv"
        )
    else:
