...
def main():
    ...
    generate_training_data_file_path = (
        CONFIG.SOURCE_CODE_DIRECTORY + "pre_process/generate_training_data.py"
    )
    ...
    attacked_ratio_nodes_list = (
        "[0.1,0.3,0.5]"  # percentage - No space shall be included in this string
    )
    attack_duration_list = (
        "[4,8,16]"  # hours - No space shall be included in this string
    )
    attack_start_times_list = (
        "[2,6,12]"  # 24-hour format - No space shall be included in this string
    )

    clean_dataset_command = "python3 " + clean_dataset_file_path + " " + str(time_step)
    generate_nodes_distance_command = (
        "python3 " + generate_nodes_distance_file_path + " " + str(time_step)
    )
    generate_nodes_pearson_correlation_command = (
        "python3 " + generate_nodes_pearson_correlation_file_path + " " + str(time_step)
    )

    generate_attack_command = (
        "python3 "
        + generate_attack_file_path
        + " "
        + str(time_step)
        ...
        + str(num_train_days)
        + " "
        + str(num_validation_days)
        + " "
        + str(num_test_days)
        + " "
        + str(k_list)
        + " "
        + str(attacked_ratio_nodes_list)
        + " "
        + str(attack_duration_list)
        + " "
        + str(attack_start_times_list)
    )

    generate_training_data_command = (
        "python3 "
        + generate_training_data_file_path
        + " "
        + str(time_step)
